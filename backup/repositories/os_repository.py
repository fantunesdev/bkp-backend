...
class OsRepository:
            ...
            for file in zip_files:
                for sub_directory in self.__backup.sub_directories:
                    if sub_directory.lower() in file:
                        target = f'{self.__backup.destiny}/{sub_directory}/{file}'
                        new_dict = {
                            'file': file,
                            'path': target
                        }
    ...
    def make_backup(self):
        self.__messages += f'Iniciando o backup da pasta "{self.__backup.origin}".\n\n'
        start = datetime.now()
        if self.__backup.need_compress:
            self.zip_sub_directories()
            self.move_zip_files()
            sub_directoryes_lenght = len(self.__backup.sub_directories)
            self.__messages += f'Total de backups realizados: {sub_directoryes_lenght}.\n'
        else:
